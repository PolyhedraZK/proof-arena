...
use halo2_xor_circuit::{circuit::XorCircuit, LOG_DEGREE};
...
fn main() -> std::io::Result<()> {
    ...
    let vk = {
        ...
        let vk = VerifyingKey::<G1Affine>::read::<_, XorCircuit>(
            &mut vk_file,
    ...
    let res = (0..repeat)
        .map(|_| {
            ...
            let res = verify_snark_gwc::<XorCircuit>(&srs, snark.clone(), &vk);
            end_timer!(timer);
            res
        })
        .all(|x| x);
